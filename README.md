...
|storj_graphs_path|Location where Monitoringdata will be Stored (Prometheus, Grafana|-|yes|
|storj_grafana_additional_networks|List of additional Dockernetworks which will be assigned to the Grafanacontainer (eg. an Proxy)|-|no|
...
|ext_port_exporter|external/exposed Port of the Exporter 9650|-|only if an exporter is Installed|
...
#Graph Stuff
storj_graphs_path: /path/to/root/monitoring/storage
...
1. Copy the generated identity files to {{ item.path }}/identity
...
### Server
You can Setup an Openvpn Server with [openvpn-install](https://github.com/angristan/

Following additional Configs have been Tested on Ubuntu 20.04.1 LTS
#### Fix Client IP
```bash
...
### Client
Dockerimage for and [openvpn-client](https://github.com/dperson/openvpn-client)
...
## Prometheus
The Prometheus configuration needs to be placed under {{ storj_graphs_path }}/prometheus/conf/prometheus.yml, then it will be mounted to the Container. If config chanegs are done, the prometheus container (_storj-prometheus_) needs to be restarted.

Example of an [Prometheus Config](docs/prometheus/example_prometheus.yml).
